...
- 問い合わせは様式プロファイル(AsciiDoc)を唯一の定義とし、xlsx・記入用
  テキスト・DDL・検証モデルを派生させてメールで受ける。Webフォームは作らない
...
- **様式は公開サイトからダウンロードできる**(build が forms-out/ を
  dist/forms/ に載せ、真正性確認用の SHA-256 を sha256.txt に併記する。
  `inquiry: {publish_forms: false}` で非公開にもできる)。様式には
  サイト識別子(`受付:` 行)が入り、他サイト向け様式の混入は受付で弾く
...
- **pending の解釈案(任意)**: AMIG_LLM_URL(OpenAI 互換のローカル LLM。
  例 `http://localhost:8000/v1/chat/completions`)を設定すると、読み取れ
  なかった様式由来メールに AI の解釈案メールが pending に並んで置かれる。
...
sites/seminar/      実証: seminar-kit の受講申込様式(定義ファイルのみ)
sites/mfg/          実証: mfg-kit の見積依頼様式(定義ファイルのみ)
```
